    ...
    return(
        <>
            ...
            <div style={{ padding: "20px" }}>
                ...
                {/* <div className="user-cards" style={{ marginLeft: marginLeft.marginLeft}}>
                    {props.data.map((item , key) => {
                        ...
                        <div className="user-card" key={key}>
                            {props.header.map((item2 , index) => {
                                return(
                                    <>
                                    <h3 key={index}>{item[item2.key]} {currentUser && item[item2.key] === currentUser.name && " (You)"}</h3>
                                    {item.image ? <img src={item[item2.key]}/> : item.image}
                                    </>
                                )
                            })}
                        <div className="actions">
                            <Link className="edit-btn text-decoration-none">edit</Link>
